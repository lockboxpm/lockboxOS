...
const ClientCard: React.FC<{
    ...
    <div
        className={`flex flex-col bg-slate-800/50 border rounded-lg transition-all duration-200 group relative overflow-hidden ${isAdmin
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3">
                        ...
                        {client.scannedData?.status && (
                            ...
                            >
                                <span
                                    className={`w-2.5 h-2.5 rounded-full ${client.scannedData.status === 'Online' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' :
                                ...
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-slate-900 border border-slate-700 rounded shadow-xl opacity-0 group-hover/status:opacity-100 group-focus/status:opacity-100 transition-opacity z-20 pointer-events-none min-w-[140px] text-center">
                                    <div className={`text-xs font-bold mb-1 ${client.scannedData.status === 'Online' ? 'text-green-400' :
                    ...
                    {client.url && client.url !== '#' && (
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-xs text-cyan-400/80 hover:text-cyan-300 font-mono flex items-center gap-1 mt-0.5"
                        >
                            {client.url.replace(/^https?:\/\//, '')}
                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                        </a>
                    )}
                    <p className="text-slate-500 text-xs mt-1">{client.role}</p>
                </div>
            </div>
